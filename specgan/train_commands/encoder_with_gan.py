...
from encoder_decoder.encoder import EncoderModel
from encoder_decoder.mlp_mapper import MlpMapper
from encoder_decoder.map_to_latent import MapToLatent
...
od_bucket_tal['24.osc2waveform'] = 4
od_bucket_tal['13.ampsustain'] = 1
od_bucket_tal['11.ampattack'] = 1
od_bucket_tal['26.lfo1waveform'] = 4
od_bucket_tal['24.osc2waveform'] = 4
od_bucket_tal['24.osc2waveform'] = 4
od_bucket_tal['12.ampdecay'] = 1
od_bucket_tal['14.amprelease'] = 1
od_bucket_tal['32.lfo1destination'] = 2
od_bucket_tal['30.lfo1amount'] = 16 
od_bucket_tal['28.lfo1rate'] = 16
od_bucket_tal['3.cutoff'] = 16
od_bucket_tal['4.resonance'] = 16

# count the number of params
param_sum: int = 0
for k,v in od_bucket_tal.items():
    param_sum += v
...
sample_input = next(dataset_iter)
sample_input= tf.expand_dims(
    sample_input, 3, name=None
)

encoder = EncoderModel(od_bucket_tal, param_sum)
map_to_latent = MapToLatent(len(od_bucket_tal) * NUMBER_OF_OUTPUTS_FROM_MAPPER, LATENT_SIZE)

encoder_ob = encoder(sample_input)

# run through mlp mappers
mlp_map_res = []
for k, v in od_bucket_tal.items():
    # x_input = tf.Variable([encoder_ob[k]], trainable=True, dtype=tf.float64)
    x_input = tf.expand_dims(
        encoder_ob[k], 0, name=None
    )
    mlp_map_res.append(np.squeeze(v(x_input).numpy()))

extracted_features = np.array(mlp_map_res).ravel()

extracted_features = tf.expand_dims(
    tf.convert_to_tensor(
        extracted_features, dtype=tf.float32
    ), 0, name=None)

latent_space = map_to_latent(extracted_features)
print (latent_space.numpy())
# load mlp mapper
